...
@router.get("/{id}", response_model=Autoridad)
def read_autoridad(id: int, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    db_obj = autoridad_service.get_autoridad(db, id)
    if db_obj is None:
        raise HTTPException(status_code=404, detail="Autoridad not found")
    return db_obj
...
@router.post("/", response_model=Autoridad)
def create_autoridad(autoridad: AutoridadCreate, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    return autoridad_service.create_autoridad(db, autoridad)

@router.put("/{id}", response_model=Autoridad)
def update_autoridad(id: int, autoridad_update: AutoridadUpdate, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    db_obj = autoridad_service.update_autoridad(db, id, autoridad_update)
    if db_obj is None:
        raise HTTPException(status_code=404, detail="Autoridad not found")
    return db_obj
...
@router.delete("/{id}", response_model=Autoridad)
def delete_autoridad(id: int, db: Session = Depends(get_db), current_user: str = Depends(get_current_user)):
    db_obj = autoridad_service.delete_autoridad(db, id)
    if db_obj is None:
        raise HTTPException(status_code=404, detail="Autoridad not found")
    return db_obj